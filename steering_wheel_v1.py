...
class SteeringController:
    def __init__(self):
        self.keys_held     = {Key.left: False, Key.right: False, Key.up: False, Key.down: False, Key.space: False, Key.shift: False}
        self.angle_history = []
        self.HISTORY_LEN   = 1
    ...
    def calibrate(self, raw_angle):
        self.center_angle = raw_angle
        self.calibrated = True
        print(f"[INFO] Steering calibrated at {raw_angle:.2f}°")
    ...
    def update(self, left_wrist, right_wrist):
        ...
        if not self.calibrated:
            self.calibrate(raw_angle_deg)

        angle = self.smooth_angle(raw_angle_deg - self.center_angle)
        ...
        if direction == "LEFT":
            normalized = min(
                ...
                max(
                    0.0,
                    (abs(angle) - DEAD_ZONE_DEG)
                    / (MAX_STEERING_DEG - DEAD_ZONE_DEG)
                )
            )
            strength = normalized ** STEERING_EXPONENT
...
def main():
    ...
    try:
        ...
        print("[WARN] Failed to set some camera properties; continuing.")
        ...
        while True:
            ...
            if results.multi_hand_landmarks and results.multi_handedness:
                ...
                for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                    ...
                    if label == "Right":
                        gesture = gesture_detector.detect(hand_landmarks, label)
                        controller.update_gesture(gesture)
                        cv2.putText(
                            frame,
                            f"Gesture : {gesture}",
                            (20, 70),
